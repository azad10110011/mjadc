...
export default function TeacherLeaveManagementPage() {
  ...
  useEffect(() => {
    Promise.all([
      api.get('/teacher/leave/summary').then((r: any) => setSummary(r.data || [])).catch(() => {}),
      api.get('/teacher/leave/applications').then((r: any) => setApplications(r.data || [])).catch(() => {}),
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      await api.post('/teacher/leave/apply', {
  ...
  const handleSaveEdit = async () => {
    ...
    setSaving(true)
    try {
      await api.put(`/teacher/leave/${editApp.id}`, {
  ...
  return (
    <PanelLayout role="teacher" title="Leave Management">
      <Card className="mb-6">
      ...
      {showForm && (
        <Card className="mb-6">
          <CardContent className="space-y-4 pt-6">
            ...
            <Input label="Reason / Description" value={reason} onChange={(e) => setReason(e.target.value)} />
      ...
      <Card>
        <CardContent className="pt-6">
          ...
          <DataTable columns={applicationColumns} data={applicationRows} emptyMessage="No applications yet" loading={loading} />
        </CardContent>
      </Card>
