      ...
      {showGrid && (
        <Card>
          <CardContent className="pt-6">
            <DataTable key={`grid-${subject}`} columns={getColumns()} data={tableData} emptyMessage="No students found for these filters" />
              ...
              {students.length > 0 && (
                <>
                  <Button variant="secondary" onClick={async () => {
                    if (!year || !cls || !examName || !subject) return
                    if (!confirm('Submit all draft results to Exam Controller for approval? You cannot edit after submission.')) return
                    try {
                      await api.post('/teacher/results/submit', { year, class: cls, exam_name: examName, subject })
                      alert('Results submitted for approval')
                      handleLoadStudents()
                    } catch (e: any) { alert(e.message) }
                  }}>Submit to Exam Controller</Button>
