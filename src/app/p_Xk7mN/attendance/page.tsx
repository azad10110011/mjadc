...
export default function AdminAttendancePage() {
  ...
  useEffect(() => {
    api.get<{ status: number; data: Teacher[] }>('/admin/attendance/teachers')
  ...
  const onTeacherChange = (teacherId: string) => {
    ...
    setStats([])
    setShowStats(false)
    const t = teachers.find((x) => String(x.id) === teacherId)
    setSubjects(t?.subjects || [])
    if (t?.subjects?.length) setSelectedSubject(t.subjects[0])
  }

  const loadStudents = async () => {
    if (!selectedSubject || !selectedClass || !selectedTeacher) return
    setLoading(true)
    setSaved(false)
    setStats([])
    setShowStats(false)
    const teacherParam = selectedTeacher ? `&teacher_id=${selectedTeacher}` : ''
    try {
      const [studRes, attRes] = await Promise.all([
        ...
        api.get<{ status: number; data: Record<number, string> }>(`/teacher/attendance/records?subject=${encodeURIComponent(selectedSubject)}&class=${encodeURIComponent(selectedClass)}&date=${selectedDate}${teacherParam}`),
      ])
  ...
  const saveAttendance = async () => {
    if (!selectedSubject || !selectedClass || !selectedDate || students.length === 0 || !selectedTeacher) return
    setSaving(true)
    try {
  ...
  const loadStats = async () => {
    if (!selectedSubject || !selectedClass || !selectedTeacher) return
    try {
      const res = await api.get<{ status: number; data: StatsRecord[] }>(`/teacher/attendance/stats?subject=${encodeURIComponent(selectedSubject)}&class=${encodeURIComponent(selectedClass)}&teacher_id=${selectedTeacher}`)
      setStats(res.data || [])
      setShowStats(true)
    } catch {
      setStats([])
  ...
  return (
    <PanelLayout role="admin" title="Attendance">
          ...
          <CardContent className="space-y-4 p-6">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
              ...
              <div className="flex items-end gap-2">
                <Button onClick={loadStudents} disabled={!selectedSubject || !selectedClass || !selectedTeacher || loading}>
                  {loading ? 'Loading...' : 'Load'}
                </Button>
                <Button variant="outline" onClick={loadStats} disabled={!selectedSubject || !selectedClass || !selectedTeacher}>
            ...
            <CardContent className="p-6">
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-lg font-semibold text-gray-800">
                  {teachers.find((t) => String(t.id) === selectedTeacher)?.name} - {selectedSubject} ({selectedClass})
