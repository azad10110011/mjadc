...
export default function AdminApproveResultPage() {
  ...
  const handlePrint = () => {
    ...
    if (!win) return
    win.document.write(`<html><head><title>${detailTitle}</title>
      <style>body{font-family:'Times New Roman',Times,serif;padding:20px}h2{text-align:center;margin-bottom:20px}table{width:100%;border-collapse:collapse;font-size:13px}th,td{border:1px solid black;padding:4px 6px;text-align:center}th{background:#f0f0f0;font-weight:bold}.left{text-align:left}</style></head><body>
      <h2>${detailTitle}</h2><table><thead><tr><th>SL No</th><th>Roll</th><th class="left">Name</th><th>MCQ</th><th>CQ</th><th>Practical</th><th>Total</th><th>Grade</th><th>GPA</th></tr></thead><tbody>
      ${detailData.map((s: any) => `<tr><td>${s.sl}</td><td>${s.student_id}</td><td class="left">${s.name}</td><td>${s.mcq}</td><td>${s.cq}</td><td>${s.practical}</td><td>${s.total}</td><td>${s.grade}</td><td>${s.gpa}</td></tr>`).join('')}
  ...
  const handleApproveBatch = async (exam_name: string, subject: string, cls: string) => {
    try {
      await api.post('/admin/results/approve-batch', { exam_name, subject, class: cls })
  ...
  const handleBackToTeacher = async (exam_name: string, subject: string, cls: string) => {
    ...
    try {
      await api.post('/admin/results/back-to-teacher', { exam_name, subject, class: cls })
    ...
    return {
      ...
      actions: (
        <div className="flex gap-2">
          ...
          {!isApproved && (
            <Button size="sm" variant="primary" onClick={() => handleApproveBatch(r.exam_name, r.subject, r.class)}>Approve All</Button>
          )}
  ...
  return (
    <PanelLayout role="admin" title="Approve Results">
      <Card>
