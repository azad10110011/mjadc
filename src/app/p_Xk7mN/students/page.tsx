...
export default function AdminStudentsPage() {
  ...
  const resetForm = () => {
    ...
    setStudentGroup(''); setSelectiveSubjects([]); setOptionalSubject(''); setBloodGroup(''); setPhotoFile(null); setExistingPhotoPath(null); setError('')
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingId) {
        await api.upload(`/admin/students/${editingId}`, formData)
  ...
  const handleView = (s: any) => {
    setViewingStudent(s); setResetPwValue(''); setResetPwResult('')
  }

  const handleResetPassword = async () => {
    if (!viewingStudent || !viewingStudent.user_id) return
    setResetPwLoading(true)
    setResetPwResult('')
    try {
      const payload: any = {}
      if (resetPwValue) payload.password = resetPwValue
      const res = await api.post<{ status: number; data: { new_password: string } }>(`/admin/users/${viewingStudent.user_id}/reset-password`, payload)
  ...
  const columns = [
    ...
    { key: 'selective_subjects', label: 'Selective Subjects' },
  ...
  const rows = students.map((s, i) => ({
    ...
    selective_subjects: (() => {
      ...
      else if (typeof raw === 'string') { try { subs = JSON.parse(raw) } catch { subs = [] } }
      return subs.length > 0
        ? <div className="flex max-w-[200px] flex-wrap gap-0.5">{subs.map((sub) => <span key={sub} className="rounded bg-gray-100 px-1.5 py-0.5 text-xs text-gray-700">{sub}</span>)}</div>
        : <span className="text-xs text-gray-400">-</span>
    })(),
    optional_subject: s.optional_subject || <span className="text-xs text-gray-400">-</span>,
  ...
  return (
    <PanelLayout role="admin" title="Student Management">
      <Card className="mb-6">
        <CardContent className="space-y-4 pt-6">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-gray-900">{editingId ? 'Edit Student' : 'Create New Student'}</h3>
          ...
          <div>
            <label className="mb-2 block text-sm font-medium text-gray-700">Compulsory Subjects (Fixed)</label>
            <div className="flex flex-wrap gap-2">
              {COMPULSORY_SUBJECTS.map((s) => (
                <span key={s} className="rounded-lg bg-blue-50 px-3 py-1.5 text-sm font-medium text-blue-700">{s}</span>
              ))}
            </div>
          </div>
          <div>
            ...
            {viewingStudent.user_id && (
              <div className="border-t border-gray-200 pt-4">
                <details className="group">
                  <summary className="cursor-pointer text-sm font-medium text-blue-600 hover:text-blue-800">Reset Password</summary>
                  <div className="mt-3 space-y-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Button variant="primary" size="sm" onClick={handleResetPassword} disabled={resetPwLoading}>
                        {resetPwLoading ? 'Resetting...' : 'Reset Password'}
                      </Button>
                    </div>
                    {resetPwResult && <p className="rounded-md bg-green-50 p-2 text-center text-sm font-medium text-green-700">{resetPwResult}</p>}
                  </div>
                </details>
              </div>
            )}
