        ...
        <CardContent className="space-y-4 pt-6">
          ...
          <div className="flex gap-2">
            ...
            {transcripts.length > 0 && (
              <Button variant="secondary" onClick={handlePrint}><Printer className="mr-1 h-4 w-4" /> Print</Button>
            )}
          </div>
                    ...
                    {t.subjects.map((s, i) => {
                      ...
                      return (
                        <tr key={i} className="border-b border-gray-200">
                          ...
                          <td className={`px-3 py-2 text-center font-medium ${s.grade === 'Absent' || s.grade === 'F' ? 'text-red-600' : ''}`}>{s.total}</td>
                          <td className={`px-3 py-2 text-center font-medium ${s.grade === 'Absent' || s.grade === 'F' ? 'text-red-600' : ''}`}>{s.grade}</td>
