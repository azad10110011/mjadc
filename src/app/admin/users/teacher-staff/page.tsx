...
export default function AdminTeacherStaffPage() {
  ...
  const fetchUsers = () => {
    api.get<{ status: number; data: User[] }>('/admin/users?roles=teacher,staff')
    ...
    <PanelLayout role="admin" title="Teacher/Staff Management">
      <Card className="mb-6">
        <CardContent className="space-y-4 pt-6">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-gray-900">{editingId ? 'Edit User' : 'Create New Teacher/Staff'}</h3>
      ...
      <Card>
        <CardContent className="pt-6">
          <h3 className="mb-4 font-semibold text-gray-900">Teacher/Staff Users</h3>
          <DataTable columns={columns} data={rows} emptyMessage="No teacher/staff users" />
        </CardContent>
      </Card>
    </PanelLayout>
  )
